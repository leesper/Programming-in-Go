...
func (write writerFunc) writeInvoice(invoice *Invoice) error {
    ...
    if err := write("INVOICE ID=%d CUSTOMER=%d DEPARTMENT=%s RAISED=%s DUE=%s "+
        "PAID=%t%s\n", invoice.Id, invoice.CustomerId, invoice.DepartmentId,
...
func (write writerFunc) writeItems(items []*Item) error {
    for _, item := range items {
        ...
        if err := write("ITEM ID=%s PRICE=%.2f QUANTITY=%d TAXBAND=%d%s\n", item.Id,
            item.Price, item.Quantity, item.TaxBand, note); err != nil {
            return err
        }
    }
    return nil
...
func parseTxtItem(lino int, line string) (item *Item, err error) {
    ...
    item.TaxBand, _ = strconv.Atoi(string(item.Id[2])) 
    return item, nil
}
